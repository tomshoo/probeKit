        ...
        for _ in range(tryct):
            try:
                ...
                if not serv:
                    if verbose:
                        Console.print(
                            f"[{BALERT}]\[*] Error: Service on port: [bold {BALERT}]{port}[/] not found[/]"
                        )
                    portstatus = False
                else:
                    portstatus = True
    ...
    def __scanner(self, port) -> str:
        ...
        if self.protocol in ["tcp", "tcp/ip", "TCP", "TCP/IP"]:
            if self.__tscanner(port):
                serv = (
                    self.__getServbyPort(port, "tcp")
                    if self.__getServbyPort(port, "tcp")
                    else "Undefined"
                )
            ...
            if self.__uscanner(port):
                serv = (
                    self.__getServbyPort(port, "udp")
                    if self.__getServbyPort(port, "udp")
                    else "Undefined"
                )
                return f"[{FSUCCESS}][+] {self.protocol}: {self.thost}: {port} is open, service: {serv}[/]"
    ...
    def display(self) -> int:
        ...
        if self.protocol in valid_protocols:
            ...
            if type == "single":
                ...
                if portstatus:
                    Console.print(portstatus)
                else:
                    print(f"{self.protocol}: {self.thost}: {single_port} is closed")
                return 0
                ...
                try:
                    ...
                    if self.threading:
                        ...
                        if type == "range":
                            output = [
                                executor.submit(self.__scanner, x)
                                for x in range(p_begin, p_end)
                            ]
                        else:
                            output = [
                                executor.submit(self.__scanner, x)
                        ...
                        if type == "range":
                            output = [
                                self.__scanner(x)
                                for x in progress.track(
                                    range(p_begin, p_end),
                                    description=f"Scanning {self.protocol}",
                                )
                            ]
                        else:
                            output = [
                                self.__scanner(int(x))
                                for x in progress.track(
                                    port["value"],
                                    description=f"Scanning {self.protocol}",
                                )
            ...
            Console.print(
                f"[{BURGENT}][**] Scan took about {round(end-start, 5)} sec(s).[/]"
            )
            return 0
