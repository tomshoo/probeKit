...
class Docs:
    ...
    def run(self):
        ...
        elif get_args(self.arguments, 0).lower() == "list":
            with open(path.join(docs_dir, "Index.md"), "r") as index:
                Console.print(markdown.Markdown(index.read()))
        elif get_args(self.arguments, 0).lower() == "show":
            if not get_args(self.arguments, 1):
                ...
                doc_name_arg = get_args(self.arguments, 1)
                if doc_name_arg not in index_list:
                    ...
                    doc_file = path.join(
                        docs_dir, doc_name_arg.replace(".", path_splitter) + ".md"
                    )
